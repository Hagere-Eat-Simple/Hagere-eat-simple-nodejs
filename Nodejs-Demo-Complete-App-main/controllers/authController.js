...
exports.login = (req, res, next) => {
    ...
    try {
		var query4 = "SELECT * FROM `customer` join `resturant` on customer.username = resturant.username WHERE resturant.username=?"
        dbConn.query(query4, [body.email], async function(error, row){
			...
			else
			{
				...
				//const checkPass = await encrypt.matchPassword(body.password, row[0].pass);
				...
				if (true) {
					...
					req.session.resturant = row[0].Rname;
					req.session.flag = row[0].flag;
